    ...
    export var ApiDefController = _module.controller('Apiman.ApiDefController',
        ['$q', '$rootScope', '$scope', '$location', 'Modals', 'PageLifecycle', 'ApiEntityLoader', 'OrgSvcs', 'Logger', '$routeParams', 'ApiDefinitionSvcs', 'Configuration', 'EntityStatusSvc', 'CurrentUser',
            ($q, $rootScope, $scope, $location, Modals, PageLifecycle, ApiEntityLoader, OrgSvcs, Logger, $routeParams, ApiDefinitionSvcs, Configuration, EntityStatusSvc, CurrentUser) => {
                ...
                $scope.isValidDefinition = (data, definitionType): IResult => {
                    switch (definitionType) {
                        case 'SwaggerJSON':
                            try {
                                ...
                                return {result : false, message: 'The specified API definition is not a valid ' + definitionType};
                            }
                        case 'WSDL':
                            try {
                                const entityRegex = /<!ENTITY\s+\w+\s+/g;
                                const externalEntityRegex = /<!ENTITY\s+\w+\s+SYSTEM|PUBLIC/g;

                                const entityMatches = data.match(entityRegex);

                                if (entityMatches && entityMatches.length > 4) {
                                    return {result: false, message: 'Potential denial of service detected: too many entities defined'};
                                }
                                const externalEntityMatches = data.match(externalEntityRegex);
                                if (externalEntityMatches) {
                                    return {result: false, message: 'External entities detected: risk of XXE'};
                                }
                                $.parseXML(data);
                                return {result : true, message: ''};
                            } catch (error) {
                ...
                $scope.saveApi = function () {
                    ...
                    const result: IResult = $scope.isValidDefinition($scope.updatedApiDefinition, $scope.selectedDefinitionType.value)
                    if (!result.result) {
                        Modals.error('Invalid API Definition!', result.message, null);
